...
export class W3mBalance extends LitElement {
  ...
  protected render() {
    ...
    return html`
      <div>
        <wcm-token-image symbol=${this.symbol}></wcm-token-image>
        <wcm-text variant="medium-regular" color="primary">${formatAmount} ${this.symbol}</wcm-text>
      </div>
    `
  }
}
